...
boss = ['valdes_95','DRP96','Locura05','TeafMaster','Hackeroto','ale9506','raydel0307',"UHTRED_OF_BEBBANBURG"]#usuarios supremos
...
async def delete_nex(client: Client, message: Message):
	...
	all_delete = await nexcloud_eliminador(user, passw, host, proxy)
	if all_delete:
...
async def webdav(filex,user_id,msg,username):
	...
	async with aiohttp.ClientSession(connector=proxy) as session:
		...
		try:
			...
			url = f"{host}s/{ids}/download?path=%2F&files={filename}"
			#send(url)
			#create txt
